...
当前身份、账号、互动与热点集成测试覆盖迁移后 ADMIN/USER 会话解析、单会话撤销、BANNED 即时失效、Redis 失败预算、注册事务、点赞/收藏并发唯一冲突、评论公开状态边界、用户治理审计、来源 advisory lock、候选审核保留、公开边界和不可变归档；只在 CI 或目标服务器隔离测试依赖中运行。
