...
本文同时描述已实现基础和 V1 目标架构。当前已有 Web、Worker、共享包、身份/权限、内容核心、公开查询/路由、互动治理、完整热点采集/审核/归档链路，以及可选 Live2D 运行时与静态回退边界；授权原创模型仍待接入。
...
看板娘开关和 R2 模型地址在服务器启动时注入，不写入镜像。根布局中的 `MascotShell` 使用 Next `connection()` 延迟到真实请求读取配置，因此当前全站路由按请求渲染；模型运行时仍不进入首屏客户端 bundle。若后续恢复静态输出，需要先设计不泄露 Secret、且支持部署时配置的公开配置端点或独立公开布局边界。
...
当前 Worker 每 60 秒锁定并发布到期文章，每 30 分钟顺序采集五个热点来源，并在启动及北京时间每日 `00:05` 检查上一自然日归档。文章发布、来源采集和日期归档分别使用 PostgreSQL 事务/锁保证重复执行安全。
