...
当前仓库已完成阶段 1 工程基础、阶段 2 身份/权限/内容核心、阶段 3 公开博客、阶段 4 用户互动/后台治理和阶段 5 每日热点代码；阶段 5 最终远端验收正在执行，目标服务器尚未部署。
...
- 当前分支已新增用户名密码注册、Turnstile、注册限流、安全会话、受保护账号页和收藏/评论活动查询。
- 事务化点赞/收藏、评论先审后发、后台评论审核，以及用户禁言、封禁、恢复、会话撤销和管理审计已由远端 PostgreSQL、Chromium 与容器门禁验证。
- 每日热点已实现五来源受限采集、样本解析、候选去重、失败隔离、后台审核、当前榜单、首页预览和北京时间不可变归档；本次分支新增的热点浏览器流程等待当前 GitHub Actions 结果。
- 当前品牌位图是原创程序化星空/漂浮书页回退，不含角色；最终原创角色和 Live2D 尚未实现。
